...
import { Icon, type VipIconName } from './Icon';
import { useTranslation } from '@/lib/i18n';

export type VipTab = 'home' | 'tiers' | 'subscribe' | 'settings';

export function BottomNav({ active, onSelect }: { active: VipTab; onSelect: (t: VipTab) => void }) {
  const { t } = useTranslation();
  const ITEMS: { key: VipTab; icon: VipIconName; label: string }[] = [
    { key: 'home',      icon: 'crown',    label: t.vip.nav.home      },
    { key: 'tiers',     icon: 'layers',   label: t.vip.nav.tiers     },
    { key: 'subscribe', icon: 'star',     label: t.vip.nav.subscribe },
    { key: 'settings',  icon: 'settings', label: t.vip.nav.settings  },
  ];
  return (
    <nav style={{
